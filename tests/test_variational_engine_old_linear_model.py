# test linear model for old variational engine
...
from pymbvi.models.mjp.linear_model import SimpleGeneExpression
...
# model_test = CompareModel(moment_initial, rates, tspan)

# forward_test = np.random.rand(100, 9)
# backward_test = np.random.rand(100, 9)
# tmp1 = model.control_gradient(forward_test, backward_test)
# tmp2 = model_test.control_gradient(forward_test, backward_test)
# err = np.linalg.norm(tmp1-tmp2, np.inf)
# print(err)
...
vi_engine.initialize(tspan, np.array([rates, rates]))
