...
from pymbvi.optimize import robust_gradient_descent, multi_arg_gradient_descent
...
vi_engine = VariationalEngine(model, obs_model, t_obs, observations, subsample=30, tspan=tspan, options=options)
...
def bound_projector(arg):
    ...
    arg[0][arg[0] > bounds[1]] = bounds[1]
    arg[1][arg[1] > bounds[1]] = bounds[1]
    # cut off lower values
    arg[0][arg[0] < bounds[0]] = bounds[0]
    arg[1][arg[1] < bounds[1]] = bounds[0]

# test optimization
#wrong_rates = vi_engine.rates+np.log(1+0.05*np.random.randn(6))
wrong_rates = vi_engine.rates.copy()
#wrong_rates[5] -= np.log(2.0)
initial_arg = [vi_engine.control.copy(), wrong_rates]
optimal_arg = multi_arg_gradient_descent(vi_engine.objective_function_inference, initial_arg, iter=100, h=1e-3)[0]
vi_engine.objective_function_inference(optimal_arg)

print(np.exp(wrong_rates))
