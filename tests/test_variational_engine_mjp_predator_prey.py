...
from pymbvi.models.mjp.specific_models import PredatorPrey
...
num_species = 2
obs_species = 1
obs_model = LognormObs(sigma, num_species, obs_species)
...
moment_initial[0:2] = initial
model = PredatorPrey(moment_initial, np.log(np.array(rates)), tspan)
...
optimal_control = robust_gradient_descent(vi_engine.objective_function, initial_control, projector=bound_projector, iter=100, h=1e-3)[0]
...
if plotting:
    plt.subplot(2, 1, 1)
    plt.plot(t_plot, states_plot[:, 0], ':g')
    ...
    plt.plot(t_plot, states_plot[:, 0], ':g')
    plt.plot(t_plot, states_plot[:, 1], ':b')
    plt.plot(t_post, states_post[:, 0], '-g')
