...
def objective_function(time, control, forward, backward, model, obs_model, obs_data):
    # bring stuff into correct shapes
    forward_flat = torch.exp(forward.reshape(-1, forward.shape[-1]))
    backward_flat = backward.reshape(-1, backward.shape[-1])
    ...
    new_forward = forward_update(odefun, time.flatten(), forward_flat)
    # forward contribution
    forward_loss = forward_contribution(time.flatten(), natural_moments, kl_eff)
    # backward contribution
    backward_loss = backward_contribution(time.flatten(), forward_flat, new_forward, backward_flat)
    # observation contribution
    obs_loss = observation_contribution(time, torch.exp(forward), obs_model, obs_data)
    loss = forward_loss+backward_loss+obs_loss
    # gradient norm stuf
    df = torch.autograd.grad(loss, forward, create_graph=True)[0]
    db = torch.autograd.grad(loss, backward, create_graph=True)[0]
    dc = torch.autograd.grad(loss, control, create_graph=True)[0]
    # print(obs_loss)
    # print(forward_loss)
    # print(backward_loss)
    # print((df**2).sum())
    # print((db**2).sum())
    # print((dc**2).sum())
    grad_loss = (df**2).sum()+(db**2).sum()+(dc**2).sum()
    #print(forward)
    return(grad_loss)
...
optimal_control = robust_gradient_descent(vi_engine.objective_function, vi_engine.control, projector=bound_projector, iter=10, h=1e-2)[0]
...
for i in range(n_iter):
    ...
    forward_test = (forward - alpha*forward.grad).detach()
    #forward_test[forward_test < 0] = 0.0
    forward_test.requires_grad_()
    backward_test = (backward - alpha*backward.grad).detach()
    backward_test.requires_grad_()
    control_test = (control - alpha*control.grad).detach()
    #bound_projector(control_test)
    control_test.requires_grad_()
    loss_test = objective_function(time, control_test, forward_test, backward_test, model, obs_model, obs_data)
    if loss_test < loss:
        accept = True
    else:
        accept = False
